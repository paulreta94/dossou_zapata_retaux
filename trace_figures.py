...
def trace_figure_nav_pontoise(x, y, x_ref, y_ref):
    ...
    with open(
        r"C:\Users\peret\Documents\00_5A\loic_davain\dossou_zapata_retaux\01-Cartes\map_ilems.png",
        "rb",
    ) as png:
    ...
    fig.update_layout(
        ...
        legend=dict(x=1, y=1),  # Position X de la légende  # Position Y de la légende
        showlegend=True,
...
def trace_figure_nav_fecamp(x, y, x_ref, y_ref):
    ...
    # Ouverture du png de la carte en mode lecture
    with open(r"C:\Users\peret\Documents\00_5A\loic_davain\dossou_zapata_retaux\01-Cartes\map_fecamp.PNG", "rb") as png:
        # Encodage en base 64 de la carte au fur et à mesure de sa lecture
        b64 = base64.b64encode(png.read())
...
def trace_figure_vitesses(t, vn, vw, vz):
    ...
    fig = make_subplots(
        ...
        cols=1,
        shared_xaxes=True,
        subplot_titles=("Vit Nord [m/s]", "Vit Ouest [m/s]", "Vit Z Up [m/s]"),
    )
    ...
    fig.update_layout(height=800, title_text="Vitesses Géographiques", title_x=0.5)
    return fig
...
def trace_erreurs_vitesse(t, err_x, err_y, err_z):
    ...
    # Mise à jour du layout général avec un titre
    fig.update_layout(height=800, title_text="Erreurs de vitesse", title_x=0.5)
    return fig
...
def trace_figure_attitudes(t, cap, rou, tan):
    ...
    fig = make_subplots(
        ...
        cols=1,
        shared_xaxes=True,
        subplot_titles=("Cap [deg]", "Roulis [deg]", "Tangage [deg]"),
    )
    ...
    # Mise à jour du layout général avec un titre
    fig.update_layout(height=800, title_text="Attitudes du Porteur", title_x=0.5)
    return fig
...
def trace_erreurs_attitude(t, err_x, err_y, err_z):
    ...
    # Mise à jour du layout général avec un titre
    fig.update_layout(height=800, title_text="Erreurs d'Attitudes", title_x=0.5)
    return fig


def trace_increments(
    t, inc_vit_x, inc_vit_y, inc_vit_z, inc_ang_x, inc_ang_y, inc_ang_z
):
    fig = make_subplots(
        rows=3,
        cols=2,
...
def trace_positions_gnss(
    ...
    """Returns a figure comparing reference position and INS one"""
    # Création de la figure avec sous-graphiques (3 lignes, 1 colonne)
    fig = make_subplots(
        rows=3,
        cols=1,
        shared_xaxes=True,
        subplot_titles=("Lat [deg]", "lon [deg]", "alt [m]"),
    )
    axis_title_font_size = 20
    ...
    fig.update_layout(
        height=600,
        title_text="Evolution de la position au cours du temps",
        font=dict(size=axis_title_font_size),
        title_x=0.5,
    )
    return fig
...
def trace_data_vordme(
    t, vor_bvs, dme_bvs, vor_dvl, dme_dvl, vor_pon, dme_pon, vor_rou, dme_rou
):
    # Création de la figure avec sous-graphiques (2 lignes, 1 colonne)
    fig = make_subplots(
        rows=2, cols=1, shared_xaxes=True, subplot_titles=("VOR [deg]", "DME [m]")
    )
    ilon = 0
    ilat = 1
    ...
    # Mise à jour du layout général avec un titre
    fig.update_layout(height=800, title_text="vordme", title_x=0.5)
    return fig


def trace_positions_vordme(
    t,
    Lr,
    Gr,
    Zr,
    ...
    fig = make_subplots(
        rows=2, cols=1, shared_xaxes=True, subplot_titles=("Lat [xx]", "lon [xx]")
    )
    ilon = 0
    ilat = 1
    ...
    fig.update_layout(
        height=800,
        title_text="Evolution de la position en fonction du temps",
        title_x=0.5,
    )
    return fig
